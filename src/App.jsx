...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchOffersFromApi = async () => {
      ...
      try {
        ...
        const nextOffers =
          apiOffers.length > 0 ? apiOffers : localOffers;

        setOffers(nextOffers);
        setSelectedOfferId((currentId) =>
          nextOffers.some((offer) => offer.id === currentId)
            ? currentId
            : nextOffers[0]?.id || null
        );
        addLog(
          apiOffers.length > 0
            ? "Teklifler API üzerinden yüklendi."
            : localOffers.length > 0
              ? "Teklifler API boş döndü, yerel veriler kullanıldı."
        ...
        if (!cancelled) {
          ...
          setSelectedOfferId((currentId) =>
            localOffers.some((offer) => offer.id === currentId)
              ? currentId
              : localOffers[0]?.id || null
          );
          setOffersError(
            "API erişilemedi. Yerel veriler gösteriliyor."
          );
  ...
  const createOffer = async (event) => {
    ...
    try {
      ...
      setSelectedOfferId((createdOffer || newOffer).id);
